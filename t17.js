    ...
    const clickButtonIfVisible = async () => {
      ...
      if (button) {
        ...
        setTimeout(async () => {
          ...
          // CookieBot Check #2
          await page.waitForTimeout(5000);  // Wait for the first 5 seconds
		  const button = await page.$('#CybotCookiebotDialogBodyButtonDecline'); // Wait for button to appear
		  if (button) { // IF button appears
			  await button.click(); // Solves CookieBot if it is present
			  console.log('Cookie clicked during the manual recovery page wait.'); // Print that it was clicked
		  }
		  
		  await page.waitForTimeout(5000);  // Wait for the remaining 5 seconds
